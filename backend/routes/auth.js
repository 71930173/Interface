...
router.post('/parent/signup',
  ...
  async (req, res) => {
    ...
    try {
      ...
      db.query('SELECT id FROM parents WHERE contact_value = ?', [contact_value], async (err, results) => {
        ...
        db.query(
          `INSERT INTO parents (first_name, last_name, contact_method, contact_value, password, language) 
           VALUES (?, ?, ?, ?, ?, ?)`,
          ...
          (err, result) => {
            ...
            const token = generateToken({ id: result.insertId, userType: 'parent' });
...
router.post('/parent/login',
  ...
  (req, res) => {
    ...
    db.query('SELECT * FROM parents WHERE contact_value = ? AND is_active = 1', [contact_value], async (err, results) => {
      if (err) return res.status(500).json({ error: 'Database error', details: err.message });
      if (results.length === 0) {
      ...
      updateLastLogin('parents', parent.id);
      const token = generateToken({ id: parent.id, userType: 'parent' });
      ...
      res.json({
        ...
        parent: {
          ...
          contact_value: parent.contact_value,
