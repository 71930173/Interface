...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    db.query(
      'SELECT id, first_name, last_name, contact_method, contact_value, language, password_hash FROM parents WHERE contact_value = ? AND is_active = 1',
      [contactValue],
      async (err, results) => {
        if (err) {
...
<head>
  ...
  <style>
    ...
    h2 { color: ${type === 'warning3min' ? '#d97706' : '#2563eb'}; margin-top: 0; }
...
<body>
  <div class="container">
    ...
    <div class="section-en">
      ...
      <h2>${type === 'warning3min' ? '⏰ Smart Queue - Your Turn is Coming Soon!' : 'Smart Queue - Appointment Confirmed'}</h2>
      <p>${type === 'warning3min' ? 'Your appointment is approaching! ⏳' : 'Your appointment details:'}</p>
      ...
      ${type === 'confirmation' ? `<p>You will receive a notification when your turn is approaching.</p>` : `<p>Please make your way to the office now to ensure you don't miss your turn.</p>`}
    ...
    <div class="section-ar">
      ...
      <h2>${type === 'warning3min' ? '⏰ Smart Queue - دورك قادم!' : 'Smart Queue - تم تأكيد الموعد'}</h2>
      <p>${type === 'warning3min' ? 'دورك يقترب! ⏳' : 'تفاصيل الموعد:'}</p>
      ...
      ${type === 'confirmation' ? `<p>سيتم إعلامك عند اقتراب دورك.</p>` : `<p>يرجى التوجه إلى المكتب الآن للتأكد من عدم تفويت دورك.</p>`}
