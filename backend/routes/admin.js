...
router.get('/dashboard', authenticateAdmin, (req, res) => {
  ...
  db.query(
    `SELECT 
      ...
      COUNT(CASE WHEN user_type = 'parent' AND status = 'served' THEN 1 END) as parents_served,
    ...
    (err, results) => {
      ...
      res.json({
        ...
        parentsToday: stats.parents_served || 0,
  ...
  try {
    ...
    db.query(sql1, (err, results) => {
      ...
      const hourlyData = results.map(r => {
        ...
        const parents = parseInt(r.parents, 10) || 0;
...
router.get('/analytics', authenticateAdmin, (req, res) => {
  ...
  db.query(
    `SELECT 
      ...
      COUNT(CASE WHEN user_type = 'parent' AND status = 'served' THEN 1 END) as parents
     FROM appointments
            ...
            (err, waitResults) => {
              ...
              res.json({
                servedOverTime: {
                  ...
                  parents: servedResults.map(r => r.parents),
                },
                staffPerformance: {
  ...
  db.query(
    `SELECT n.*, 
      CASE 
        ...
        WHEN n.recipient_type = 'parent' THEN CONCAT(p.first_name, ' ', p.last_name)
     ...
     LEFT JOIN parents p ON n.recipient_type = 'parent' AND n.recipient_id = p.id
...
router.get('/export/:type', authenticateAdmin, async (req, res) => {
  ...
  const allowedTypes = ['appointments', 'stats', 'staff', 'students', 'parents'];
    ...
    switch(type) {
      ...
      case 'parents':
        query = `SELECT id, first_name, last_name, contact_method, contact_value, language, is_active, created_at 
         FROM parents WHERE is_active = 1 ORDER BY created_at DESC`;
        break;
    }
