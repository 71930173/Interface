  ...
  db.query(
    ...
    (err, results) => {
      ...
      res.json({
        ...
        parent_priority_count: waiting.filter(w => w.priority === 1).length,
  ...
  db.getConnection((err, connection) => {
    ...
    connection.beginTransaction(async (err) => {
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          const recipientId = appointment.user_type === 'student' ? appointment.student_id : appointment.parent_id;
        ...
        try {
          ...
          await sendTurnNotification(
            ...
            appointment.user_type === 'student' ? appointment.student_id : appointment.parent_id,
                                  ...
                                  db.query(
                                    ...
                                    (err, staffResults) => {
                                      ...
                                      sendTurnNotification(
                                        appt.user_type,
                                        appt.user_type === 'student' ? appt.student_id : appt.parent_id,
                                        appt.contact_method,
                                        appt.contact_value,
                                        appt.first_name,
                                        appt.ticket_number,
                                      ...
                                      const recipientId = appt.user_type === 'student' ? appt.student_id : appt.parent_id;
                                      const message = '🎉 It is Your Turn Now! / دورك الآن!\n\n' +
                                        'English: Please go to the office now, the Dr. is waiting for you!\n' +
                                        'العربية: اذهب إلى المكتب الآن، Dr. في انتظارك!\n\n' +
...
// sends notification via parent's preferred channel (email/WhatsApp/SMS)
    ...
    connection.beginTransaction((err) => {
      ...
      connection.query(
        ...
        (err, results) => {
          ...
          const appointment = results[0];
          const recipientId = appointment.user_type === 'student' ? appointment.student_id : appointment.parent_id;
          const parentLang = appointment.user_language || 'en';
          const isAr = parentLang === 'ar';
...
تم حل مشكلتك (تذكرة #${appointment.ticket_number}) عن بعد من قبل موظفنا. لا حاجة لزيارة المكتب.${noteText}`;
              ...
              if (contactMethod === 'email' || (contactValue && contactValue.includes('@'))) {
                ...
                const emailSubject = isAr 
                  ? `=?UTF-8?B?${Buffer.from('تم حل مشكلتك - Smart Queue / Your Issue Has Been Resolved').toString('base64')}?=`
                  : 'Smart Queue - Your Issue Has Been Resolved / تم حل مشكلتك';
    ...
    <div class="section-ar">
      ...
      <p class="footer">إشعار آلي من نظام Smart Queue.</p>
              ...
              if (ULTRAMSG_INSTANCE_ID && ULTRAMSG_TOKEN && appointment.user_phone) {
                ...
                const whatsappMessage = `✅ *Smart Queue Update / تحديث Smart Queue*
...
function notifyWaitingUsers(staffId, notifyType) {
  ...
  db.query(
    `SELECT a.id, a.student_id, a.parent_id, a.user_type,
    ...
    async (err, waitingUsers) => {
      ...
      if (waitingUsers && waitingUsers.length > 0) {
        for (const user of waitingUsers) {
          ...
          const parentLang = user.user_language || 'en';
          const isAr = parentLang === 'ar';
...
` +
          ...
          if (emailTo) {
            const emailSubject = isPausedNotify
              ? (isAr
                ? `=?UTF-8?B?${Buffer.from('⏸️ الموظف متوقف مؤقتاً - Smart Queue / Staff Paused').toString('base64')}?=`
                : '⏸️ Smart Queue - Staff Paused / الموظف متوقف مؤقتاً')
              : (isAr
                ? `=?UTF-8?B?${Buffer.from('✅ الموظف متاح مرة أخرى - Smart Queue / Staff Available').toString('base64')}?=`
                : '✅ Smart Queue - Staff Available Again / الموظف متاح مرة أخرى');
      ...
      <p class="footer">إشعار آلي من نظام Smart Queue.</p>
          ...
          if (phoneTo && process.env.ULTRAMSG_INSTANCE_ID && process.env.ULTRAMSG_TOKEN) {
            const waMessage = isPausedNotify
              ? `⏸️ *Smart Queue Alert / تنبيه Smart Queue*
...
` +
                `Thank you! / شكراً!`
              : `✅ *Smart Queue Update / تحديث Smart Queue*
...
router.get('/stats', authenticateStaff, (req, res) => {
  ...
  db.query(
    `SELECT 
      ...
      COUNT(CASE WHEN a.user_type = 'parent' AND a.status = 'served' AND ${servedDateFilter} THEN 1 END) as parents_served,
    ...
    (err, statsResults) => {
      ...
      db.query(
        `SELECT 
          ...
          COUNT(CASE WHEN a.user_type = 'parent' THEN 1 END) as parents,
          COUNT(*) as total
         FROM appointments a
                ...
                (err, recentResults) => {
                  ...
                  res.json({
                    ...
                    summary: {
                      ...
                      parentsServed: stats.parents_served || 0,
                    ...
                    servedOverTime: {
                      ...
                      parents: timeResults.map(r => r.parents || 0),
