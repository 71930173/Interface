    ...
    <div className="min-h-screen bg-gradient-to-br from-dark-900 via-dark-800 to-dark-900 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">{lang === 'ar' ? 'دخول المسؤول' : 'Admin Login'}</h1>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" name="email" value={formData.email} onChange={handleChange}
                  ...
                  className={`w-full pl-12 pr-4 py-3 bg-white/10 border-2 rounded-xl text-white placeholder-white/40 focus:outline-none ${errors.email ? 'border-danger-400' : 'border-white/20 focus:border-dark-400'}`} />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange}
                  className={`w-full pl-12 pr-12 py-3 bg-white/10 border-2 rounded-xl text-white placeholder-white/40 focus:outline-none ${errors.password ? 'border-danger-400' : 'border-white/20 focus:border-dark-400'}`} />
            ...
            <motion.button type="submit" disabled={isLoading} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              className="w-full py-3 px-4 rounded-xl bg-dark-600 text-white font-semibold hover:bg-dark-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 shadow-glow">
